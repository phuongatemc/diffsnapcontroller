...
type ChangedBlockLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ChangedBlock, err error)
...
// List lists all ChangedBlocks in the indexer.
func (s *changedBlockLister) List(selector labels.Selector) (ret []*v1alpha1.ChangedBlock, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ChangedBlock))
	})
	return ret, err
}
...
type ChangedBlockNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ChangedBlock, error)
...
// List lists all ChangedBlocks in the indexer for a given namespace.
func (s changedBlockNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ChangedBlock, err error) {
...
// Get retrieves the ChangedBlock from the indexer for a given namespace and name.
func (s changedBlockNamespaceLister) Get(name string) (*v1alpha1.ChangedBlock, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("changedblock"), name)
	}
	return obj.(*v1alpha1.ChangedBlock), nil
}
