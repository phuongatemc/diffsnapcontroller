...
type ChangedBlockLister interface {
	...
	List(selector labels.Selector) (ret []*cbt.ChangedBlock, err error)
...
// List lists all ChangedBlocks in the indexer.
func (s *changedBlockLister) List(selector labels.Selector) (ret []*cbt.ChangedBlock, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*cbt.ChangedBlock))
	})
	return ret, err
}
...
type ChangedBlockNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*cbt.ChangedBlock, error)
...
// List lists all ChangedBlocks in the indexer for a given namespace.
func (s changedBlockNamespaceLister) List(selector labels.Selector) (ret []*cbt.ChangedBlock, err error) {
...
// Get retrieves the ChangedBlock from the indexer for a given namespace and name.
func (s changedBlockNamespaceLister) Get(name string) (*cbt.ChangedBlock, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(cbt.Resource("changedblock"), name)
	}
	return obj.(*cbt.ChangedBlock), nil
}
